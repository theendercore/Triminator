...
export default function Generator({}: { path: string }) {
    ...
    return (
        <div class="container">
            ...
            <div
                class="p-10 bg-text bg-opacity-5 rounded-3xl md:grid md:grid-cols-2 md:gap-10 flex flex-col gap-5 w-full ">
                ...
                <div
                    ...
                    <div>
                        ...
                        <p class="flex-wrap">{packData.description || "Pack generated by Triminator"}</p>
                    </div>

                    <div class="min-w-[8rem] min-h-[8rem] rounded-3xl bg-background-2 grid items-center">
                        <img
                            src={(packData.icon !== null) ? URL.createObjectURL(packData?.icon) : "/img/dr_doof.png"}
                            alt={packData.icon?.name} height={128} width={128} class="pixel-art rounded-3xl"/>
                    </div>
                ...
                <div className="text-xl text-center md:col-span-2 md:place-self-center flex gap-6 ">
                    <SecondaryButton onClick={() => setDownloadState("DATA")} fromField="header" type="submit">
                        Download Datapack
                    </SecondaryButton>
                    <SecondaryButton onClick={() => setDownloadState("RESOURCE")} fromField="header" type="submit">
                        Download Resource Pack
                    </SecondaryButton>
                    <PrimaryButton onClick={() => setDownloadState("BOTH")}
                                   fromField="header" type="submit">
                        Download Both
                    </PrimaryButton>

                </div>

                <a class="hidden" download={`${formatName(packData.name)}Datapack.zip`} ref={dpLink} href={dpUrl}></a>
                <a class="hidden" download={`${formatName(packData.name)}ResourceRack.zip`} ref={rpLink}
                   href={rpUrl}></a>

                {devMode &&
                    <>
                        <CodePre className="col-span-2">{format(packData)}</CodePre>
                        <PrimaryButton className=" top-32 fixed left-3" onClick={() => {
                            setPackData({
                                ...packData,
